...
export function Sketching() {
  ...
  const sketchingProducts = useMemo(() => {
    const filtered = products.filter((p) => p.type?.toLowerCase() === "sketching");
  ...
  return (
    <div className="min-h-screen py-12 px-4 lg:px-20">
      <div className="max-w-[1440px] mx-auto">
        ...
        {/* 🔥 Fixed: was rendering stale `filtered`, now renders sorted `sketchingProducts` */}
        {sketchingProducts.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {sketchingProducts.map((product) => (
